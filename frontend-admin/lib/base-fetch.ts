...
export default async function baseFetch<T>(endpoint:string,options: RequestInit = {}): Promise<Response<T>> {
  ...
  try {
    ...
    try {
      ...
      if (response.ok) return { status: "success", message: data.mensaje ?? data.message ?? messageOk,statusCode:statusCode ,data:data };
      return { status: "error", message: data.mensaje ?? JSON.stringify(data.detail) ?? data.message ?? messageError, statusCode };
    } catch {
